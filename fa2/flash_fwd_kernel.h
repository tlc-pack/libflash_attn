...
template<typename Kernel_traits, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, typename Params>
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    if ((Is_causal || Is_local || !Is_even_MN) && n_block_max <= n_block_min) {
        ...
        flash2::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/false>(
    ...
    flash2::copy<Is_even_MN, Is_even_K>(gmem_tiled_copy_QKV, tQgQ, tQsQ, tQcQ, tQpQ,
    ...
    flash2::copy<Is_even_MN, Is_even_K>(gmem_tiled_copy_QKV, tKgK, tKsK, tKVcKV, tKVpKV,
    ...
    flash2::Softmax<2 * size<1>(acc_o)> softmax;

    const float alibi_slope = !Has_alibi || params.alibi_slopes_ptr == nullptr ? 0.0f : reinterpret_cast<float *>(params.alibi_slopes_ptr)[bidb * params.alibi_slopes_batch_stride + bidh] / params.scale_softmax;
    flash2::Mask<Is_causal, Is_local, Has_alibi> mask(binfo.actual_seqlen_k, binfo.actual_seqlen_q, params.window_size_left, params.window_size_right, alibi_slope);
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            ...
            flash2::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, tVgV, tVsV, tKVcKV, tKVpKV);
        } else {
            // Clear the smem tiles to account for predicated off loads
            flash2::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/true>(
                gmem_tiled_copy_QKV, tVgV, tVsV, tKVcKV, tKVpKV, binfo.actual_seqlen_k - n_block * kBlockN
            );
        }
        cute::cp_async_fence();

        flash2::gemm</*A_in_regs=*/Kernel_traits::Is_Q_in_regs>(
        ...
        if (n_block > n_block_min) {
            ...
            flash2::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, tKgK, tKsK, tKVcKV, tKVpKV);
        ...
        Tensor rP = flash2::convert_type<Element>(acc_s);

        // Reshape rP from (MMA=4, MMA_M, MMA_N) to ((4, 2), MMA_M, MMA_N / 2)
        // if using m16n8k16 or (4, MMA_M, MMA_N) if using m16n8k8.
        Tensor tOrP = make_tensor(rP.data(), flash2::convert_layout_acc_Aregs<Kernel_traits::TiledMma>(rP.layout()));
        // if (cute::thread0()) { print(tOrP); }
        flash2::gemm_rs(acc_o, tOrP, tOrVt, tOsVt, tiled_mma, smem_tiled_copy_V, smem_thr_copy_V);
    ...
    Tensor rO = flash2::convert_type<Element>(acc_o);
...
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    if constexpr (Append_KV) {
        ...
        for (int n_block = n_block_max - 1; n_block >= n_block_copy_min; n_block--) {
            flash2::copy_w_min_idx<Is_even_K>(
            ...
            if (params.rotary_dim == 0) {
                ...
                if (params.is_rotary_interleaved) {
                    ...
                    flash2::copy_rotary_interleaved<Is_even_K, /*Clear_OOB_K=*/false>(
                    ...
                    // Don't clear OOB_K because we're writing to global memory
                    flash2::copy_rotary_contiguous<Is_even_K, /*Clear_OOB_K=*/false>(
    ...
    Tensor rO = flash2::convert_type<ElementO>(acc_o);
...
inline __device__ void compute_attn(const Params &params) {
    ...
    flash2::compute_attn_1rowblock<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K>(params, bidb, bidh, m_block);
}
...
inline __device__ void compute_attn_splitkv(const Params &params) {
    ...
    flash2::compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Split, Append_KV>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
}
...
inline __device__ void combine_attn_seqk_parallel(const Params &params) {
    ...
    for (int split = 0; split < params.num_splits; ++split) {
        flash2::copy</*Is_even_MN=*/false, Is_even_K>(
    ...
    Tensor rO = flash2::convert_type<Element>(tOrO);
    // Write to gO
    #pragma unroll

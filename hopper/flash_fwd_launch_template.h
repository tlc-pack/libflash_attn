...
void run_flash_fwd(Flash_fwd_params& params, cudaStream_t stream) {
  ...
  using CollectiveMainloop = flash3::CollectiveMainloopFwd<Kernel_traits, Is_causal, Seqlen_traits>;
  using CollectiveEpilogue = flash3::CollectiveEpilogueFwd<Kernel_traits, Seqlen_traits>;
  using Scheduler = std::conditional_t<
      Seqlen_traits::kUseVarSeqLen, flash3::SingleTileScheduler,
      std::conditional_t<!Is_causal, flash3::StaticPersistentTileScheduler,
                         flash3::DynamicPersistentTileScheduler<
                             Kernel_traits::kNThreads - cutlass::NumThreadsPerWarpGroup,
                             Kernel_traits::NumProducerThreads>>>;
  // using Scheduler = flash3::SingleTileScheduler;
  ...
  if constexpr (cutlass::sizeof_bits_v<Element> == 8)
    kernel = (void*)flash3::compute_attn_ws_fp8<Kernel_traits, Is_causal, Scheduler, Seqlen_traits>;
  else
    kernel = (void*)flash3::compute_attn_ws<Kernel_traits, Is_causal, Scheduler, Seqlen_traits>;
  ...
  // printf("smem_size = %d, q = %d, k = %d, v = %d, o = %d.\n", smem_size, smem_size_q,
  // smem_size_k, smem_size_v, smem_size_o);
...
void run_mha_fwd_hdim64(Flash_fwd_params& params, cudaStream_t stream) {
  ...
  BOOL_SWITCH(params.is_causal, Is_causal, [&] {
    SEQLEN_SWITCH(params.cu_seqlens_q, Seqlen_traits, [&] {
      run_flash_fwd<Flash_fwd_kernel_traits<Headdim, 192, 128, 16, 2, false, 1, T>, Is_causal,
  ...
  BOOL_SWITCH(params.is_causal, Is_causal, [&] {
    SEQLEN_SWITCH(params.cu_seqlens_q, Seqlen_traits, [&] {
      ...
      BOOL_SWITCH(cutlass::ceil_div(params.seqlen_q, 128) % 2 == 0 && !Is_causal &&
                  ...
                  UseCluster, [&] {
                    run_flash_fwd<Flash_fwd_kernel_traits<Headdim, 128, Is_causal ? 128 : 176, 12,

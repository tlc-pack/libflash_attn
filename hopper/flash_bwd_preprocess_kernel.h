...
public:
    ...
    operator()(Params const& params, [[maybe_unused]] char* smem_buf) {
        ...
        flash3::copy</*Is_even_MN=*/false, /*Is_even_K=*/false, /*Clear_OOB_MN=*/true>(
        ...
        Tensor o_fp32 = flash3::convert_type<float>(make_tensor(tOrO.data(), l));
        Tensor do_fp32 = flash3::convert_type<float>(make_tensor(tOrdO.data(), l));
        ...
        for (int mi = 0; mi < size<0>(o_fp32); ++mi) {
            ...
            flash3::SumOp<float> sum_op;
            dP_sum(mi) = flash3::Allreduce<kGmemThreadsPerRow>::run(dP_sum_cur, sum_op);
        }
